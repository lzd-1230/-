...
void pre_travel(struct tree* pt)
{
	if(pt!=NULL)							//非空指针才需要printf data域			
	{
		printf("%c\n",pt->data);
		if(pt->plchild!=NULL)
...
void last_travel(struct tree* pt)
{
	if(pt!=NULL)						//若空指针才需要printf data域			
	{
		if(pt->plchild!=NULL)
			pre_travel(pt->plchild);
		if(pt->prchild!=NULL)
...
//!!!注意创建的时候输入的一定要是满二叉树,因为递归算法的出口是NULL
void pre_travel(ptree pt)
{
	if(pt!=NULL)						//若不为空指针才需要printf data域			
	{	
			printf("%3c",pt->data);
		if(pt->lchild!=NULL)
...
int yezi(ptree pt)
{
	if(pt==NULL)	//空就返回0
		return 0;
	if(pt->lchild== NULL && pt->rchild== NULL)		//!!!注:这里不能写成两个== 
		return 1;
	else
...
void ChangeLR(ptree& pt)
{
	...
	if ((pt->lchild->data=='#' && pt->rchild->data=='#'))
	{
		return;
	}
